...
%       .Card_in_Axis   : 计算 Card 和 Oblique 比例时，分母是 Axis（true）还是总数（false，默认）
...
%   xWin : 1 × nWin 窗口中心位置向量
    ...
    if ~isfield(cfg, 'doSmooth'), cfg.doSmooth = false; end
    ...
    if strcmp(cfg.mode, 'continuous')
        ...
        TseriesCard = sum(bin_counts_all(:,:,1:4:16), 3) ./ sum(bin_counts_all(:,:,ternary(cfg.Card_in_Axis,1:2:16,1:16)), 3);  
        TseriesObli = sum(bin_counts_all(:,:,3:4:16), 3) ./ sum(bin_counts_all(:,:,ternary(cfg.Card_in_Axis,1:2:16,1:16)), 3);  
    else % 'block'
        for bi = 1:nBlocks
            ...
            for t0 = 1:nWinBlock
                ...
                blockCard(si, t0, bi) = sum(bin_counts(si,1:4:16)) ./ sum(bin_counts(si,ternary(cfg.Card_in_Axis,1:2:16,1:16)));  % 1:2:16
                blockObli(si, t0, bi) = sum(bin_counts(si,3:4:16)) ./ sum(bin_counts(si,ternary(cfg.Card_in_Axis,1:2:16,1:16)));  % 1:2:16
            end
        end
