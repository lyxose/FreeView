function plot_violin_prop(data, colors, labels, varargin)
    % 小提琴图版本的plot_bar_prop，数据为比例，展示分布特征
    % data: nsbj x 1 或 nsbj x 2
    % colors: nBars x 3
    % labels: cell array, nBars
    % varargin: 可选参数，支持 'ylabel', 'xlabel', 'title', 'showInd', 'showIndNum', 'showIndLink', 'chanceLevel', 'chanceLabel'
    cfg = struct(varargin{:});
    [nSub, nB] = size(data);
    if isempty(colors)
    ...
    % 绘制chance level基线
    yline(chanceLevel, '--', 'Color', [0.4 0.4 0.4], 'LineWidth', 1.2, 'Label', cfg.chanceLabel, ...
    ...
    if isfield(cfg,'showInd') && cfg.showInd
        ...
        if nB == 2
            ...
            % 计算配对 Cohen's d
            vals1 = data(mask_pair,1);
            vals2 = data(mask_pair,2);
            diffs = vals1 - vals2;
    ...
    if nB == 1
        d = (mean(data(mask_in,1),'omitnan') - chanceLevel) / std(data(mask_in,1),'omitnan');
    end
    ...
    y_max = max([chanceLevel, max(m+se), max(data_valid(:)), max(xi_all(:))+bump, max(data(:))]);
    ...
    if nB < 3 && exist('d','var')
        ...
        x_lim = ax.XLim;
        y_lim = ax.YLim;
        xpos = x_lim(2) - 0.02 * diff(x_lim);
        ypos = y_lim(2) - 0.04 * diff(y_lim);
        text(xpos, ypos, sprintf('d'' = %.2f', d), ...
