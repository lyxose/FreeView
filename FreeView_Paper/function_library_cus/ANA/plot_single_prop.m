function plot_single_prop(series, xWin, color, label, cfg)
    ...
    % 绘制chance level基线
    yline(ax, chanceLevel, '--', 'Color', [0.4 0.4 0.4], 'LineWidth', 1.2, 'Label', 'Gap', 'LabelHorizontalAlignment','left');
    ...
    legend(ax, {[label,'±SE'], label, 'Gap'}, 'Location', 'northeastoutside');
    set(ax, 'FontSize', 12, 'LineWidth', 1.2);

    % 自动调整y轴范围
    y_min = min([chanceLevel, min(m-se)]);
    y_max = max([chanceLevel, max(m+se)]);
