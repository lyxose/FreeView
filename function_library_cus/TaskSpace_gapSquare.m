function [Eccent, Orient, x, y, elementClusterTags] = TaskSpace_gapSquare(Side_region, Width_region, Gap_region, trialNum, noiseP, scWidth, scHeight, bgWidth, rot_ang, tgSeed)
...
ap = diff(xc); % max edge length in pix
wp = diff(yc); % edge width
vertices_x = [xc(1) xc(1)    xc(2)    xc(2)]+scWidth/2;
vertices_y = [yc(2) yc(2)-ap yc(2)-ap yc(2)]+scHeight/2;
ROI_matrix = poly2mask(vertices_x, vertices_y, scHeight, scWidth);

% remove central
vertices_xr = [xc(1) xc(1)    xc(2)    xc(2)]+ [wp,wp,-wp,-wp] +scWidth/2;
vertices_yr = [yc(2) yc(2)-ap yc(2)-ap yc(2)]+ [-wp,wp,wp,-wp] +scHeight/2;
ROI_matrix = ROI_matrix & ~poly2mask(vertices_xr, vertices_yr, scHeight, scWidth);
...
if sum(xc)/2<min(xcg) || sum(xc)/2>max(xcg)
...
maskLeft = ROI_matrix & X<sum(xc)/2  +scWidth/2;
maskRight = ROI_matrix & X>=sum(xc)/2+scWidth/2;
...
maskLeft  = imrotate(maskLeft , rot_ang, 'bilinear', 'crop');
maskRight = imrotate(maskRight , rot_ang, 'bilinear', 'crop');
...
% tag all samples
mask1 = maskLeft(indices);
mask2 = maskRight(indices);
elementClusterTags = mask1 * 1 + mask2.*(~mask1)*2;
