...
### 异常情况
#### 12 XuShuhua
测试阶段，右边的一个角基本找不到。从下一个被试开始改用分区quest策略。
#### 13 LiFangyuan
即便没有找到目标，被试也可能按键盘猜，导致实际难度可能比预先较高一些。
代码逻辑错误，以至于实际上是由quest4（背景噪声组）来生成的contrast难度，需要前置421~425行到第310行代码前（从#14起已更正）
#### 15 FuChangchao
270个trial左右，location lost了一次，主试通过遮挡摄像头重新捕捉后恢复正常
340个trial左右，可能是疲劳效应比较明显，错误率较高。
460个trial左右，发现有一些误判的trial或者error略高过1°的trial，可能是后期错误率较高的原因
#### 16 LiNuo
眼镜框比较粗，镜片较厚，靠上的注视点导致眼睛反光被遮挡，无法顺利启动实验。但是任务空间不受影响，因此提示被试可以在注视点附近小范围扫视来启动实验。但并不work，最终通过推眼镜解决，但是一定程度上影响了校准精度，学习阶段错误率较高。实验结束后没有及时按空格退出PTB，导致下一个被试开始前卡死了一次，但数据保存正常。
#### 17 BaoHongri
100个trial附近，突然lost location，后面又好了
在300 trial左右的时候申请休息了5分多钟

### 被试表现及反馈
#### 12 XuShuhua
从注视点开始向圆的内部找；觉得左上角概率较高，但是并没有专门去搜左上角。
#### 13 LiFangyuan
发现有的位置是连续重复的；做完整体感觉外周比中间多，左上和右边多，明显感觉到是不均匀的分布，但是没有依此规律进行搜索。
#### 14 hanbeilei
左右扫视，认为一帧可能有多个目标。觉得是外周多中间少的环状分布。
#### 15 FuChangchao
学习阶段发现在左上、左下、右上、右下这四个角比较多（实际分布没有右上右下），所以会从这四个角开始找。
#### 16 LiNuo
被试发现左上角，左下角，右边正中比较多，而且发现最外圈很少，中间很少，主要出现在半径的中间两个4分位范围。 测试阶段被试会放弃找特别黑或特别白的区域。
...
### 被试表现及反馈
#### 17 BaoHongri
一开始比较简单，一眼就能看见。如果找不到，他才会从上到下找一遍，然后从左往右找一遍。被试觉得目标没有什么空间分布规律，随机出现。
#### 18 HeChuan
策略1：从右边开始找，绕一圈。策略2：从上一次发现的位置开始找。
发现目标有可能在相近位置重复出现，但是不超过两次。觉得目标大概率出现在近似1/2半径的环形的区域上，标记了五个热点
建议：每一个block都进行一次难度渐进的训练，否则会丢失对目标的印象，导致整个block都很难找到。
#### 19 JiYao
先整体看，然后从上往下、从左往右，会优先关注背景中的线条纹理。练习阶段发现一象限靠近中央的位置经常遇到，其他位置比较均匀（有画草图）。但是在搜索时没有特别关注这个区域。
...
### quest反馈标准
quest结构体现以注视准确为正确，其余均为错误进行更新。但是当被试校准不好时，很多trial中，被试找到了target，但是误差略大于1°即被判错，导致阈限偏高。改用被试按键反馈作为quest正确率更新标准可能更好（从#13被试开始，已经更正）

### quest分区域
分区域控制对比度，保证每个区域都有相同概率被看见（从#13被试开始，已经更正）
